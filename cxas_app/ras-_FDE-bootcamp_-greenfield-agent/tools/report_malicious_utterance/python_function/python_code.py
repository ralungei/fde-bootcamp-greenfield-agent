def report_malicious_utterance(category: str = "abuse", summary: str = "") -> dict:
    ...
    utt_low = str(context.state.get("utterance") or "").strip().lower()
    ...
    if len(utt_low) < 25 and not any(w in utt_low for w in ("ignore", "prompt", "system", "drop ", "select ", "rm -", "kill", "bomb", "hurt", "die", "hack")):
        return {
            "status": "ignored",
            "message": "A short question, apology, or interruption is NOT a malicious utterance.",
            "agent_action": "Do NOT terminate the call. Repeat 'This call may be recorded for quality and training purposes.' if the opening greeting was interrupted, and ask how you can help the caller.",
        }
